...
compress_model_name='facebook/bart-large-cnn'
#retriever = db.as_retriever(search_kwargs={"k": 4}, search_type="mmr")
retriever = db.as_retriever(search_kwargs={"k": 8})
compress_pipe = pipeline("summarization", model=compress_model_name, max_length=130, min_length=60, do_sample=False)
compress_llm = HuggingFacePipeline(pipeline=compress_pipe)
compressor = LLMChainExtractor.from_llm(compress_llm, prompt=prompt)
...
def compress_document(doc):
    # Create a new Document with the same metadata and compressed page_content
    # Ensure that page_content is a string
    return Document(metadata=doc.metadata, page_content=str(compress_pipe(doc.page_content)))

# COMMAND ----------

compressed_docs = [compress_document(doc) for doc in docs]
